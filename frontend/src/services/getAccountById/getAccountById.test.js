...
describe('getAccountById', () => {
    ...
    it('should throw an error when the network response is not ok', async () => {
        jest.spyOn(global, 'fetch').mockRejectedValueOnce(new Error('Network response was not ok'));

        await expect(getAccountById('404')).rejects.toThrow('Network response was not ok');
    });
